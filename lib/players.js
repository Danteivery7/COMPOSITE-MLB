...
export function rateBatter(stats, gp = 0) {
    ...
    const paFactor = Math.min(1.0, pa / 30); // Need ~30 PA for full confidence
...
export function ratePitcher(stats, position, gp = 0) {
    ...
    // Relievers need ~15 IP, Starters need ~30 IP to build full confidence
    const ipTrustThreshold = isReliever ? 15 : 30;
...
export function computePlayerRating(rawStats, isPitcher, position = '', playerId = null, careerRaw = null, playerAge = null) {
    ...
    if (isPitcher || isPitcher === 'two-way') {
        ...
        const ipTrust = seasonResult.ipFactor ?? 1.0;
        
        if (careerResult) {
            let cw = getCareerWeight(gp);
            // If sample is small, trust season less, rely more on career
            cw = 1.0 - ((1.0 - cw) * ipTrust);
            ...
            // No career data + small sample = regress towards base 50 OVR
            finalPitchRating = Math.round(seasonResult.rating * ipTrust + 50 * (1 - ipTrust));
        }
        ...
        if (hasBatting && (isOhtani || isPitcher === 'two-way')) {
            ...
            const paTrust = batResult.paFactor ?? 1.0;
            
            if (careerBatResult) {
                let cw = getCareerWeight(batGP);
                cw = 1.0 - ((1.0 - cw) * paTrust);
                ...
                finalBatRating = Math.round(batResult.rating * paTrust + 50 * (1 - paTrust));
            }
