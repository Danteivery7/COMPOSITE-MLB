...
export function computePlayerRating(rawStats, isPitcher, position = '', playerId = null, careerRaw = null, playerAge = null) {
    ...
    if (isPitcher || isPitcher === 'two-way') {
        ...
        // Blend season + career (with age decay on career)
        let finalPitchRating = seasonResult.rating;
        if (careerResult) {
            const cw = getCareerWeight(gp);
            const decayedCareer = Math.round(careerResult.rating * ageDecay);
        ...
        if (hasBatting && (isOhtani || isPitcher === 'two-way')) {
            ...
            if (careerBatResult) {
                ...
                const decayedCareer = Math.round(careerBatResult.rating * ageDecay);
    ...
    // Blend (with age decay on career)
