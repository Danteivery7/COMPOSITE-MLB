...
export function generatePlayerAnalysis(player, current, career, gameLogs = []) {
    ...
    // ── PARAGRAPH 3: Smarter Projections & Outlook ──────────────────
    let p3 = "";
    if (isOnIL) {
        ...
        // SMARTER PROJECTION LOGIC: Weighted blend of current rate and career average
        ...
        // Weight: Give more weight to career baseline early in the season (low cp)
        const careerWeight = Math.max(0.2, 1 - (cp / 50)); // Fade out career influence after 50 games
        const blendedHRRate = (currentHRRate * (1 - careerWeight)) + (careerHRRate * careerWeight);
        const projectedHR = Math.round((current.HR || 0) + (blendedHRRate * gamesRemaining));

        p3 = `If these early trends hold, ${name} is projected to finish the year with approximately ${projectedHR} home runs. By blending his current output with his .${Math.round(careerHRRate * 1000 * 162/1000)} HR/season career baseline, this model accounts for both his ${experience} capabilities and the remaining ${gamesRemaining} games in the schedule.`;
    }
