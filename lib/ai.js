...
export function generatePlayerAnalysis(player, current, career, gameLogs = [], statusLabel = '', bStats = {}, pStats = '', accoladeText = '', teamGP = 0) {
    ...
    if (isOnIL) {
        ...
        if (isPitcher) {
            ...
            // ── DYNAMIC 5-POINT PERFORMANCE ASSESSMENT (Last 15-20 Appearances) 
            let streakNote = '';
            if (gameLogs.length > 0) {
                const r = gameLogs.slice(0, 20); // Using 20 as the max window
                
                // Cumulative Stats for Accuracy
                ...
                const curOBA = tIP > 0 ? tH / (tIP * 3) : (pStats.OBA || 0.250); // Estimating batters faced
                const curKBB = tBB > 0 ? tK / tBB : (tK > 0 ? 5 : 2.5);

                // Baselines
                const bERA = isRookie ? 4.20 : (career?.ERA || 4.20);
                const bWHIP = isRookie ? 1.35 : (career?.WHIP || 1.30);
                const bK9 = isRookie ? 8.0 : (career?.['K/9'] || 8.5);
                const bOBA = isRookie ? 0.260 : (pStats.OBA || 0.250);
                const bKBB = isRookie ? 2.0 : 3.0;

                // 5-Point Consensus
                let score = 0;
                if (curERA < bERA) score++;
                if (curWHIP < bWHIP) score++;
                if (curK9 > bK9) score++;
                if (curOBA < bOBA) score++;
                if (curKBB > bKBB) score++;

                if (score >= 4) {
                    streakNote = ` He is currently sizzling on the mound, with his last string of outings outperforming his career benchmarks across almost every major category.`;
                } else if (score === 3) {
                    streakNote = ` He is delivering a remarkably steady performance of late, tracking right in line with the high expectations of his career profile.`;
                } else if (score >= 1) {
                    streakNote = ` He's navigated a challenging cold patch recently, with his underlying metrics trailing his established historical performance levels.`;
                } else {
                    streakNote = ` He is effectively in a slump on the mound, with his recent form failing to meet his established career benchmarks across the board.`;
                }
            }
            ...
            // ── DYNAMIC 5-POINT PERFORMANCE ASSESSMENT (Last 15-20 Games) 
            let streakNote = '';
            if (gameLogs.length > 0) {
                const r = gameLogs.slice(0, 20);
                
                // Cumulative Data for Precision
                ...
                const t2B = r.reduce((s, l) => s + (parseFloat(l.stats?.[3]) || 0), 0);
                const t3B = r.reduce((s, l) => s + (parseFloat(l.stats?.[4]) || 0), 0);

                const curAVG = tAB > 0 ? tH / tAB : (current.AVG || 0.248);
                const curOBP = (tAB + tBB) > 0 ? (tH + tBB) / (tAB + tBB) : (current.OBP || 0.315);
                const curSLG = tAB > 0 ? (tH + t2B + (2 * t3B) + (3 * tHR)) / tAB : (current.SLG || 0.400);
                const curOPS = curOBP + curSLG;
                const curHits = tH / Math.max(1, r.length);

                // Baselines
                const bAVG = isRookie ? 0.248 : (career?.AVG || 0.248);
                const bOBP = isRookie ? 0.315 : (career?.OBP || 0.315);
                const bSLG = isRookie ? 0.405 : (career?.SLG || 0.400);
                const bOPS = isRookie ? 0.720 : (career?.OPS || 0.720);
                const bHits = (isRookie ? 112 : (career?.H || 112)) / 162; 

                // 5-Point Consensus
                let score = 0;
                if (curAVG > bAVG) score++;
                if (curOBP > bOBP) score++;
                if (curSLG > bSLG) score++;
                if (curOPS > bOPS) score++;
                if (curHits > bHits) score++;

                if (score >= 4) {
                    streakNote = ` He is currently sizzling at the plate, with his multi-stat profile outperforming his established benchmarks across almost every category.`;
                } else if (score === 3) {
                    streakNote = ` He is delivering a remarkably steady performance of late, tracking right in line with the high expectations of his career profile.`;
                } else if (score >= 1) {
                    streakNote = ` He's navigated a challenging cold patch recently, with his underlying production at the plate trailing his historical standards.`;
                } else {
                    streakNote = ` He is effectively in a slump at the plate, with his recent production falling significantly below his established career benchmarks.`;
                }
            }
