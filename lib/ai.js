...
export function generatePlayerAnalysis(player, current, career, gameLogs = [], statusLabel = '', bStats = {}, pStats = {}) {
    ...
    const isTwoWay = (player.isTwoWay || String(player.id) === '39832') && Object.keys(bStats).length > 0 && Object.keys(pStats).length > 0;
    ...
    if (isOnIL) {
        ...
        if (isPitcher || isTwoWay) {
            ...
            p3 = `Looking ahead, projections suggest ${name} will settle into a ${xERA.toFixed(2)} ERA range with a projected ${xK9.toFixed(1)} K/9 rate as the workload increases. ${isTwoWay ? 'Coupled with his offensive production, he remains a perennial MVP frontrunner.' : 'By blending his current command with his veteran baseline, this model accounts for a realistic evolution of his arsenal across the 162-game grind.'}`;
        } else {
