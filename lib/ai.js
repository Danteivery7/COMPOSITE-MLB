 ...
 * Aware of the 3-layer rating system:
...
export function generatePlayerAnalysis(player, current, career, gameLogs = [], statusLabel = '', bStats = {}, pStats = {}) {
    ...
    const careerReliability = Math.min(1.0, careerGP / 200); // 0..1
    ...
    if (isOnIL) {
        ...
        if (isPitcher) {
            ...
            if (isEstablished) {
                p1 = `${fullName} enters the 2026 season as a proven ${pos === 'SP' ? 'rotation arm' : 'bullpen weapon'} with a ${fmt2(careerERA)} career ERA across ${fmt2(careerIP)} innings of MLB service. At ${age} years old, this ${experience} ${pos} is expected to be a key piece of the ${player.teamAbbr || ''} pitching plans. `;
            } else if (isRookie) {
            ...
            if (isEstablished) {
                p1 = `${fullName}, a ${experience} ${pos} with a ${fmt3(careerOPS)} career OPS and ${fmtInt(careerHR)} home runs over ${careerGP} games, steps into 2026 as a cornerstone of the ${player.teamAbbr || ''} lineup. `;
            } else if (isRookie) {
