    ...
    if (isOnIL) {
        ...
        if (isPitcher) {
            ...
            if (gameLogs.length > 0) {
                ...
                const recentER = recentLogs.reduce((sum, log) => sum + (parseFloat(log.stats?.[1]) || 0), 0);
                const recentIP = recentLogs.reduce((sum, log) => sum + (parseFloat(log.stats?.[0]) || 0), 0);
                const recentERA = recentIP > 0 ? (recentER / recentIP) * 9 : 4.20;
            ...
            if (gameLogs.length > 0) {
                const recentLogs = gameLogs.slice(0, 15);
                const recentH = recentLogs.reduce((sum, log) => sum + (parseFloat(log.stats?.[1]) || 0), 0);
                const recentAB = recentLogs.reduce((sum, log) => sum + (parseFloat(log.stats?.[0]) || 0), 0);
                const recentAVG = recentAB > 0 ? recentH / recentAB : 0;
