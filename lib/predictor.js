...
export async function predict(teamAId, teamBId, options = {}) {
    ...
    // ── Hottness / Momentum adjustment ───────────────────────────────
    // A small but realistic factor based on recent streaks/hotScore
    const hotA = teamA.hotScore || 0;
    const hotB = teamB.hotScore || 0;
    const hotDiff = (hotA - hotB) / 100; // Normalized momentum diff
    ...
    // Blended weight: OVR provides baseline, Hottness provides the "surge" factor
    aExpectedRuns += (ovrDiff * 0.4 + hotDiff * 0.6) * formEmphasis;
    bExpectedRuns -= (ovrDiff * 0.4 + hotDiff * 0.6) * formEmphasis;
...
function buildWhyBullets(a, b, aWinPct, neutralSite) {
    ...
    if (Math.abs(hotA - hotB) > 20) {
        const hotter = hotA > hotB ? a : b;
        bullets.push(`${hotter.name} carries significant momentum into this matchup (Hottness: ${hotter.hotScore.toFixed(0)})`);
    } else if (Math.abs(hotA - hotB) < 10 && (a.streakNum > 1 && b.streakNum > 1)) {
        bullets.push('Both teams are currently surging — expect a high-momentum battle');
    }
