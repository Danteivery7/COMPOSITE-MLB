        ...
        for (const group of data.children || []) {
            for (const division of group.children || []) {
                for (const entry of division.standings?.entries || []) {
                    ...
                    // Strict Opening Day Record Check: If total games played (W+L) is > 10, it's definitely ST data.
                    const rawWins = parseInt(stats.wins) || 0;
                    const rawLosses = parseInt(stats.losses) || 0;
                    const isStale = (rawWins + rawLosses) > 10;
                    const forceZero = isPreseason || isStale;
                    ...
                    teams[team.id] = {
                        ...
                        streak: forceZero ? '-' : stats.streak || '-',
                        last10: forceZero ? '0-0' : stats.record || '',
                        gamesPlayed: forceZero ? 0 : (rawWins + rawLosses),
                    };
                }
            }
        }
...
export async function fetchPlayerStats(athleteId) {
    ...
    try {
        ...
        // Smart Unlock: If the year is older than current, it's definitely an old season.
        // We will then look at individual splits to find "Regular Season" for the current year.
        const isOldSeason = statYear < currentYear;
        ...
        splits.forEach(s => {
            ...
            // STAT SYNC: If this split is NOT "Regular Season" for the current year, we must zero it out.
            // This prevents Spring Training stats or 2025 residual from appearing as active 2026 data.
            const isRegSeason = splitName.includes('regular') && splitName.includes(String(currentYear));
            const shouldZero = isOldSeason || !isRegSeason;
            ...
            if ((isBattingSplit || (identifiesAsBatting && isShohei)) && (!stats.batting.AVG || isRegSeason)) {
                // If we found a regular season split, it overwrites. 
                // If we only have a stale split, and stats.batting is empty, it populates with 0s.
...
export async function fetchPostGameSummary(gameId, winningAbbr) {
    try {
        ...
        return { winningPitcher, losingPitcher, savingPitcher, pog };
    } catch(e) { 
