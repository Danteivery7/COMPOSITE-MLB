...
export async function fetchStandings() {
    const cached = cacheGet('espn_standings_v2');
    if (cached) return cached;
    ...
    try {
        ...
        for (const group of data.children || []) {
            for (const division of group.children || []) {
                for (const entry of division.standings?.entries || []) {
                    ...
                    const isSTGroup = groupName.includes('Cactus') || groupName.includes('Grapefruit') || groupName.includes('Spring');
                    
                    const rawWins = parseInt(stats.wins) || 0;
                    const rawLosses = parseInt(stats.losses) || 0;
                    const gp = rawWins + rawLosses;

                    // If it's a Spring Training group, or if the scoreboard says we are in preseason,
                    // we only zero out if the game count suggests we haven't hit Opening Day yet.
                    const forceZero = isSTGroup || (isPreseason && gp > 30);
                    ...
                    teams[team.id] = {
                        ...
                        streak: forceZero ? '-' : (stats.streakSummary || stats.streak || '-'),
                        last10: forceZero ? '0-0' : (stats.record || stats.last10 || ''),
        ...
        cacheSet('espn_standings_v2', result, CACHE_TTL.STATS);
