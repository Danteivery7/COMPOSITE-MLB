...
export async function fetchStandings() {
    const cached = cacheGet('espn_standings_v3');
    if (cached) return cached;
    ...
    try {
            ...
            for (const division of group.children || []) {
                for (const entry of division.standings?.entries || []) {
                    ...
                    teams[team.id] = {
                        ...
                        streak: forceZero ? '—' : (stats.streak_summary || stats.streakSummary || stats.streak || '—'),
        ...
        cacheSet('espn_standings_v3', result, CACHE_TTL.STATS);
